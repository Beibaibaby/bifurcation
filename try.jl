...
const threshee_init = 0.92
const threshie_init = 0.7
...
function run_bifurcation_analysis(; threshee_fixed=threshee_init, threshie_start=threshie_init)
    ...
    #save the plot
    savefig(p1, "./plots_bif/equilibrium_branch.png")
    display(p1)
    ...
    if !isempty(hopfpts)
        ...
        display(p2)
        #save plot
        savefig(p2, "./plots_bif/limit_cycle_branch.png")
        println(br_po)
    end
end
...
function main()
    ...
    run_bifurcation_analysis(threshee_fixed=0.9, threshie_start=0.1)
end
